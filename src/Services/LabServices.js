...
export const createLab = async ({ institute_id, lab_name = "", lab_address = "", lab_admin_name = "", lab_student_capacity = "50" }) => {
    lab_student_capacity = Number(lab_student_capacity);
    institute_id = Number(institute_id)
    let data = await axios.post('/lab', {institute_id, lab_student_capacity, lab_address, lab_name, lab_admin_name });
    return data.data
}
